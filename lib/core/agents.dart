  ...
  "storyboards": [
    {
      ...
      "video_prompt": "这一整段连续镜头的运镜与动作（中文），严格按下文 ①~⑥ 结构写。",
      "duration": 8
    }
  ]
}
...
②【连续动作链】最关键：用「先…→紧接着…→随后…→最后…」把整段动作写成一条不间断的时间线，显式写出动作之间的物理因果（抽象示例：「主体A 做出动作并因某原因导致后果X → 紧接着触发 B 的反应Y → 同一瞬间道具Z 因受力发生状态变化 → 最后各自收尾」）；关键道具全程连续，不能凭空有无。
③【单一运镜】整段只用一种主运镜，单独成句（如「镜头始终低角度侧面跟拍主体，随其移动平稳横摇」）；绝不要在一个 beat 里切换多种机位或描述剪切。
④【环境】地点、年代、天气、前景/背景关键元素。
⑤【光影】光源方向、色温、质感。
⑥【负面约束】结尾固定加一句：「避免人物变形、肢体粘连穿模、人物瞬移或凭空出现消失、画面跳切闪烁、违反重力与惯性、画面出现任何字幕/文字/水印」。
- 动作要具体可拍、符合重力；一个 beat 只承载一条连续动作链，可包含完整事件（起因→过程→反应→收尾），因为是一整段连续视频不会被切断。
- 凡剧本写到某角色"离开/进入/前往某处"，video_prompt 必须写成**实际完成的连续位移**（转身→迈步→走向目标→穿过/进入→远去），明确人物移动并最终离开画面或抵达目标；绝不能写成站在原地不动或只给一个静态结束姿势。
