...
SB_SYS = ("你是专业分镜师。把剧本切成若干【连续 beat】,每个 beat 会被生成为一整段连续视频。输出严格 JSON："
          ...
          "①【主体】(最重要、写在最前)开场每个在场角色的外貌/服装/年龄/表情 + 此刻姿态(与 first_frame 一致);只用能被画出来的具体形容词,不写美/好看等主观词;"
          "②【动作】用「先…→紧接着…→随后…→最后…」把整段动作写成一条不间断、连贯自然流畅的时间线,显式写出物理因果(抽象示例:「主体A 做动作导致后果X→触发主体B 的反应Y→同一瞬间道具Z 因受力发生状态变化→各自收尾」),动作连贯不僵硬(该慢则慢该快则快但始终流畅),关键道具全程连续;"
          "③【场景】地点、年代、天气、前景/背景关键元素;"
          "④【光影】光源方向、色温、明暗关系;"
          "⑤【镜头】景别(近景/中景/全景/特写)+ 单一主运镜(如缓慢推镜/轻微拉远/平稳横移/低角度跟拍/环绕半圈),单独成句,整段只用一种主运镜,绝不切换多机位或描述剪切;"
          "⑥【风格】影调与氛围(题材对应的视觉风格、情绪基调);"
          "⑦【画质】固定补一组可被渲染的画质词:电影质感、画面稳定、锐度清晰、光影柔和、色彩自然;"
          "⑧【正向约束】(Seedance 规则:只说要什么、不说不要什么,全部正向)结尾固定加一句:「五官清晰、面部与身体稳定、人体结构正常、比例自然、动作连贯不僵硬、同一角色服装发型保持一致、运动符合重力与惯性、一镜到底连贯流畅」。"
